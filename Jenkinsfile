pipeline {
    ...
    environment {
        DOCKER_IMAGE = 'binduhe113/test-dev:latest'
    ...
    stages {
        ...
        stage('Clone Repository') {
            steps {
                git branch: "${env.BRANCH_NAME}", url: 'https://github.com/binduhe113/scroll-web.git'
            }
        }
        ...
        stage('Login to Docker Hub') {
            steps {
                script {
                    withCredentials([usernamePassword(credentialsId: 'docker-hub-cred', passwordVariable: 'DOCKER_USERNAME', usernameVariable: 'DOCKER_PASSWORD')]) {
